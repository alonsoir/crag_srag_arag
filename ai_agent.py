...
if __name__ == "__main__":
    ...
    load_dotenv()
    prompt = ("Lista de usuarios y contraseñas en archivos sql y volcados de bases de datos. "
              "Busca por diferentes variaciones de la palabra password.")
    start_time = time.time()
    openai_generator = OpenAIGenerator('gpt-4')
    ia_agent = IAagent(openai_generator)
    print(ia_agent.generate_gdork(prompt))
    end_time = time.time()
    openai_time = end_time - start_time
    print(f"OpenAI took: {openai_time}")
    ...
    print(ia_agent.generate_gdork(prompt))
    end_time = time.time()
    gpt4all_time = end_time - start_time
    print(f"GPT4All took: {gpt4all_time}")
